...
class Pubtwo extends Base {
    ...
    public function index(){
        ...
        $lists = Db::table($tbname)->field('id,sort,title,checked,addtime,sign,is_ok')->where('is_delete',0)->order('sort asc,addtime desc')->select();
    ...
    public function update(){
        if($this->request->isPost()){
            ...
            $list = Db::name($tbname)->field('id,title,content')->where('id',$data['id'])->find();
